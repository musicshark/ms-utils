...
def process_note(session_dict, session_note_dict, midi_input):
    global note_tracker_dict
    global note_pressed

    hex_note = f"0x{midi_list[0]}"

    if (int('0x90', 16) <= int(hex_note, 16) <= int('0x9F', 16)):
        note_start = time.time()
        note_tracker_dict.update({midi_input[1] : note_start})
        note_input = find_note(midi_input)
#        print(f'note start {note_tracker_dict}')
        
        note_pressed = True
        return note_input
    
    #if (int('0x80', 16) <= int(hex_note, 16) <= int('0x8F', 16)) and len(session_dict) == 0:
    #    return 

    if (int('0x80', 16) <= int(hex_note, 16) <= int('0x8F', 16)) and len(session_dict) != 0:
        note_end = time.time()
        n = note_tracker_dict.get(midi_input[1])
        if n == None:
            total_note_time = 0

        else:
#            print(f'note end {n}')
            total_note_time = note_end - n
            note_input = find_note(midi_input)

        session_dict["total_notes"][note_input].times_played += 1
        session_dict["total_notes"][note_input].total_time += total_note_time
        
        session_note_dict["total_notes"][note_input].times_played += 1
        session_note_dict["total_notes"][note_input].total_time += total_note_time

        note_tracker_dict.pop(midi_input[1])

        session_dict["total_notes_played"] += 1
        session_dict["note_time_played"] += total_note_time

        session_note_dict["total_notes_played"] += 1
        session_note_dict["note_time_played"] += total_note_time

        note_pressed = False
        #print(f'note end {n}\t{total_note_time}')
        return note_input#, total_note_time

    else:
        pass


def find_note(midi_input):
    match (int(midi_input[1], 16) % 12):
        case 0:
            return 'C'
        case 1:
            return 'C#'
...
def sum_results_from_two_tables(cursor1, cursor2, col, table1, table2):
    res1 = cursor1.execute(f"""SELECT {col} FROM {table1};""")
    res2 = cursor2.execute(f"""SELECT {col} FROM {table2};""")
    
    results1_list = res1.fetchall()
    results2_list = res2.fetchall()

    results1 = results1_list[0]
    results2 = results2_list[0]
    
    if debug:
        print(f"results1: {results1} - {table1} - {col}")
        print(f"results2: {results2} - {table2}")

    new_total = results1[0] + results2[0] 
    if debug:
        print(f"new_total: {new_total}")
   
    return new_total

def sum_notes_from_two_tables(cursor1, cursor2, col, source_table, dest_table, dest_conn):
    global debug 
    res1 = cursor1.execute(f"""SELECT {col} FROM {source_table};""")
    res2 = cursor2.execute(f"""SELECT {col} FROM {dest_table};""")
    ...
    if debug == True:
        print(f'results1: {results1_list} - {col} - {source_table} \nresults2: {results2_list} - {col} - {dest_table}')

    for n in range(1, 13):    
        results1 = results1_list[n]
        results2 = results2_list[n]

        new_total = results1[0] + results2[0]
        new_dest_rows.append(new_total)

    tuple_dest_col = tuple(new_dest_rows)
    
    if debug == True:
        print(f"tuple_dest_col: {tuple_dest_col} - {col}")
        print(f"col: {col}  {type(col)}")

    dest_conn.commit()

    return tuple_dest_col

def subtract_results_from_session_LIFETIME_and_current_session_dict(cursor, col, table, current_session_value):
    res1 = cursor.execute(f"""SELECT {col} FROM {table}""")
    results1_list = res1.fetchall()
    results1 = results1_list[0]

    if debug:
        print(f"results1: {results1} - {table} - {current_session_dict}")

    new_total = current_session_value - results1[0]
    if debug:
        print(f"new_total: {new_total} - {table}")
   
    return abs(new_total)

def subtract_notes_from_session_LIFETIME_and_current_session_dict(cursor, col, table, current_session_dict):
    global debug 
    res1 = cursor.execute(f"""SELECT {col} FROM {table};""")
    ...
    if debug == True:
        print(f'results1: {results1_list} - {table} - {current_session_dict}')

    new_dest_rows = list()

    db_notes_index = 1
    if debug:
        print(f"current_session_dict: {current_session_dict['total_notes'].keys()}")
        print(f"col: {col}")
    match col:
        case "note_times_played":
            for k, v in current_session_dict["total_notes"].items():
                results1 = results1_list[db_notes_index]
                new_total = v.times_played - results1[0]
                new_dest_rows.append(abs(new_total))

                if debug: 
                    print(f"new_total: {abs(new_total)}")

                
        case "note_time_total":
            for k, v in current_session_dict["total_notes"].items():
                results1 = results1_list[db_notes_index]
                new_total = v.total_time - results1[0]
                new_dest_rows.append(abs(new_total))
    ...
    if debug == True:
        print(f"tuple_dest_col: {tuple_dest_col}")
        print(f"col: {col}  {type(col)}")

    return tuple_dest_col

def add_session_totals_to_db(current_session_dict, db_filepath):
    global debug
    try:
        db_conn = sqlite3.connect(db_filepath)

        db_cursor = db_conn.cursor()
        session_cursor = db_conn.cursor()
        if debug:
        ...
        if db_filepath == "./db/LIFETIME.db" or db_filepath == "db/LIFETIME.db":
            name_res = session_cursor.execute(f"""SELECT name FROM current_session;""")
            LIFETIME_prev_session_name = name_res.fetchall()
            LIFETIME_prev_session_name_list = LIFETIME_prev_session_name[0]

            if int(LIFETIME_prev_session_name_list[0]) < int(session_name):
                session_cursor.execute("""DROP TABLE current_session;""")
                db_conn.commit()
                notes_dict = note_dict_init()
                initalize_LIFETIME_current_session_table(session_cursor, session_name, notes_dict, db_conn)

            new_TNP = subtract_results_from_session_LIFETIME_and_current_session_dict(session_cursor, "total_notes_played", "current_session", session_total_notes_played)
            new_TTP = subtract_results_from_session_LIFETIME_and_current_session_dict(session_cursor, "total_note_time_played", "current_session", session_total_note_time_played)
            new_runtime = subtract_results_from_session_LIFETIME_and_current_session_dict(session_cursor, "runtime", "current_session", session_runtime)
            new_note_times_played_tuple = subtract_notes_from_session_LIFETIME_and_current_session_dict(session_cursor, "note_times_played", "current_session", current_session_dict)
            new_note_time_total_tuple = subtract_notes_from_session_LIFETIME_and_current_session_dict(session_cursor, "note_time_total", "current_session", current_session_dict)
            
            new_values = (new_TNP, new_TTP, new_runtime)
            if debug:
                print(f"new_values: {new_values}\nnew_note_times_played_tuple: {new_note_times_played_tuple}\nnew_note_time_total_tuple: {new_note_time_total_tuple}")

            update_session_totals_to_db(db_cursor, "current_session", new_values, new_note_times_played_tuple, new_note_time_total_tuple)
            add_session_totals_to_LIFETIME(session_cursor, db_cursor, db_conn)

        else:
            new_TNP = sum_results_from_two_tables(session_cursor, db_cursor,"total_notes_played", "day_totals", f"session_{session_name}")
            new_TTP = sum_results_from_two_tables(session_cursor, db_cursor,"total_note_time_played", "day_totals", f"session_{session_name}")
            new_runtime = sum_results_from_two_tables(session_cursor, db_cursor, "runtime", "day_totals", f"session_{session_name}")
            new_note_times_played_tuple = sum_notes_from_two_tables(session_cursor, db_cursor, "note_times_played", "day_totals", f"session_{session_name}", db_conn)
            new_note_time_total_tuple = sum_notes_from_two_tables(session_cursor, db_cursor, "note_time_total", "day_totals", f"session_{session_name}", db_conn)
            session_end = current_session_dict["session_end"]
        
            new_values = (new_TNP, new_TTP, new_runtime, session_end)
            if debug:
                print(new_values)

            update_session_totals_to_db(db_cursor, "day_totals", new_values, new_note_times_played_tuple, new_note_time_total_tuple)

        if debug:
            print(f"day_total_values: {new_values}")

        db_conn.commit()
    except Exception as e:
        print(f"EXCEPTION CAUGHT add_session_totals_to_db: {e}")
...
def update_session_totals_to_db(db_cursor, table, new_values, new_note_times_played_tuple, new_note_time_total_tuple):
    new_TNP = new_values[0]
    new_TTP = new_values[1]
    new_runtime = new_values[2]
    
    db_cursor.execute(f"UPDATE {table} SET total_notes_played = {new_TNP};")
    db_cursor.execute(f"UPDATE {table} SET total_note_time_played = {new_TTP};")
    db_cursor.execute(f"UPDATE {table} SET runtime = {new_runtime};")

    if table == "day_totals":
        current_day_end = new_values[3]
        db_cursor.execute(f"UPDATE {table} SET end = {current_day_end};")
    
    tuple_index = 0
    note_dict = note_dict_init()
    for n in note_dict.keys():
        note_name = n
        new_note_times_played = new_note_times_played_tuple[tuple_index]
        new_note_time_total = new_note_time_total_tuple[tuple_index]

        db_cursor.execute(f"""UPDATE {table} 
                            SET note_times_played = ? 
                            WHERE note_name = ?""", (new_note_times_played, note_name))
            
        db_cursor.execute(f"""UPDATE {table} 
                            SET note_time_total = ? 
                            WHERE note_name = ?""", (new_note_time_total, note_name))
        tuple_index += 1

def add_session_totals_to_LIFETIME(session_cursor, db_cursor, db_conn):
    try:
        new_TNP = sum_results_from_two_tables(session_cursor, db_cursor,"total_notes_played", "current_session", "LIFETIME")
        new_TTP = sum_results_from_two_tables(session_cursor, db_cursor,"total_note_time_played", "current_session", "LIFETIME")
        new_runtime = sum_results_from_two_tables(session_cursor, db_cursor, "runtime", "current_session", "LIFETIME")
        new_note_times_played_tuple = sum_notes_from_two_tables(session_cursor, db_cursor, "note_times_played", "current_session", "LIFETIME", db_conn)
        new_note_time_total_tuple = sum_notes_from_two_tables(session_cursor, db_cursor, "note_time_total", "current_session", "LIFETIME", db_conn)
    
        new_values = (new_TNP, new_TTP, new_runtime)
        update_session_totals_to_db(db_cursor, f"LIFETIME", new_values, new_note_times_played_tuple, new_note_time_total_tuple)

    except Exception as e:
        print(f"EXCEPTION CAUGHT add_session_totals_to_LIFETIME - {e}")
        dispatcher["menu"](db_day_filepath, db_LIFETIME_filepath)
...
def find_session_end_and_runtime(db_cursor):
    global db_LIFETIME_filepath
    current_session_name = get_previous_session_name(db_LIFETIME_filepath)

    res1 = db_cursor.execute(f'''SELECT start 
                                FROM {current_session_name}''')
    session_start_list = res1.fetchall()
    session_start_tuple = session_start_list[0]

    session_end = time.time()
    runtime = session_end - session_start_tuple[0]

    return session_end, runtime
...
def start_session(db_day_filepath, db_LIFETIME):
    ...
    s_start = str(f"{session_start.strftime('%H%M%S')}")

    current_session_dict = init_current_session_dict() 
    current_session_note = init_current_session_dict()
...
if __name__ == "__main__":
    ...
    #current_session_dict = init_current_session_dict()
    #initalize_session_stats_to_db(current_session_dict, db_day_filepath)

    s_start = str(f"{datetime.now().strftime('%H%M%S')}")

    #midi_list.append(ser.read().hex())           

    # Make sure list is len(3) and check if note_on
   # if len(midi_list) == 3:
    #    start_session(db_day_filepath, db_LIFETIME_filepath)

    #current_session_dict_queue = queue.Queue()
