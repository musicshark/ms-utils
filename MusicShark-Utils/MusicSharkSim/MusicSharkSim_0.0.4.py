...
def increment_total_stats_to_table(current_session_note_dict, db_cursor, col, db_name):
    ...
    if col == "total_notes_played":
        new_total = res1_tuple[0] + current_session_note_dict["total_notes_played"]
    
    elif col == "total_note_time_played":
        new_total = res1_tuple[0] + current_session_note_dict["note_time_played"]
    ...
    db_cursor.execute(f"""UPDATE {table_name}
                        SET {col} = {new_total} WHERE ROWID = 1;""")
...
def update_table_with_runtime_and_session_end(db_cursor, session_end, runtime, table_name):
    ...
    db_cursor.execute(f'''UPDATE {table_name} SET runtime = ? WHERE ROWID = 1;''', (runtime_total, ))

    if table_name == 'day_totals' or table_name == previous_session_name:
        db_cursor.execute(f'''UPDATE {table_name} SET end = ? WHERE ROWID = 1;''', (session_end, ))
...
def start_session(db_day_filepath, db_LIFETIME):
    ...
    current_session_dict["session_end"] = current_session_dict["note_time_played"] + random.uniform(2.1, 32.8)
    session_end = current_session_dict["session_end"]
    ...
    #session_name = current_session_dict["session_name"]
    #update_os_env(session_name)

    #os.environ["TEST"] = "music_shark_sim-test"
    #print(os.environ["TEST"])
    #dispatcher['menu'](db_day_filepath, db_LIFETIME_filepath)

    update_session_tracker_db(db_day_filepath, db_day_date, str(f"session_{current_session_dict['session_name']}"))
...
# Hash the most recent version of db_day, add it as primary key of session_upload_tracker_table
# Add most recent session as second column in session_upload_tracker_table
# 
# Have API caller program delete this entry after it's been added to internet
def update_session_tracker_db(db_day_filepath, db_day_name, current_session_name, max_retries=5, base_delay=0.05):
    global SESSION_UPLOAD_TRACKER_DB_PATH
    sql_insert_statment = ("INSERT INTO session_upload_tracker (db_day_hash, db_day_name, most_recent_session, has_been_sent) VALUES (?, ?, ?, ?);")
    ...
    try:
        upload_tracker_cursor = upload_tracker_conn.cursor()
        upload_tracker_cursor.execute(sql_insert_statment, (db_day_hash, db_day_name, current_session_name, "false"))
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"{SESSION_UPLOAD_TRACKER_DB_PATH}"):
        db_create_filepath(version,SESSION_UPLOAD_TRACKER_DB_PATH)
