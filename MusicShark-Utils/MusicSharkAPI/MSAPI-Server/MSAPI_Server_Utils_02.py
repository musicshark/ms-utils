...
def db_create_filepath(db_dir_path, db_name):
    ...
    if not os.path.exists(f"./db/{db_dir_path}"):
        subprocess.run(["mkdir", "-p", f"./db/{db_dir_path}"])
...
#        cursor.execute(init_day_db, day_data)
        ...
        initalize_totals_db(notes_dict, "day_totals", conn)
        conn.commit()
...
# Initalize db_day or LIFETIME_db
...
def init_db_total(current_session_dict, db_cursor):
    try: 
        session_name = current_session_dict["session_name"]
        session_total_notes_played = current_session_dict["total_notes_played"]
        session_total_note_time_played = current_session_dict["note_time_played"]
...
#            print(type(current_session_dict["total_session_runtime"]))
        ...
        cursor.execute(f"""INSERT OR REPLACE INTO {current_session_table}
...
def update_session_note_values_to_db(notes_dict, current_session_table, db_day_cursor):
    try:
        ...
        for note_name in notes_dict.keys():
            ...
            db_day_cursor.execute(f"""INSERT OR REPLACE INTO {current_session_table}
                              SET note_times_played = {note_times_played} WHERE note_name = {note_times_played}""")

            # Update note_time_total/note
            db_day_cursor.execute(f"""INSERT OR REPLACE INTO {current_session_table}
                              SET note_time_total = {note_time_total} WHERE note_name = {note_name}""")
...
def increment_note_stats_to_table(current_session_note_dict, current_note, db_cursor, col, db_name):
    ...
    db_cursor.execute(f"""INSERT OR REPLACE INTO {table_name} ({col})
                        VALUES {new_total};""")
                        #WHERE note_name = {current_note};""")
...
def increment_total_stats_to_table(current_session_note_dict, db_cursor, col, db_name):
    ...
    db_cursor.execute(f"""INSERT OR REPLACE INTO {table_name}
                        SET {col} = {new_total};""")
...
def update_table_with_runtime_and_session_end(db_cursor, session_end, runtime, table_name, db_LIFETIME_filepath):
    ...
    db_cursor.execute(f'''INSERT OR REPLACE INTO {table_name} SET runtime = {runtime_total};''')

    if table_name == 'day_totals': #or table_name == previous_session_name:
        db_cursor.execute(f'''INSERT OR REPLACE INTO {table_name} SET end = {session_end};''')
...
#    sum_session_db_and_totals_db(current_session_dict, db_LIFETIME_cursor, "total_notes_played", "1")
#
#    sum_session_db_and_totals_db(current_session_dict, db_day_cursor, "total_note_time_played", "1")
#    sum_session_db_and_totals_db(current_session_dict, db_LIFETIME_cursor, "total_note_time_played", "1")
#
#    sum_session_db_and_totals_db(current_session_dict, db_LIFETIME_cursor, "runtime", "1")
#    sum_session_db_and_totals_db(current_session_dict, db_LIFETIME_cursor, "runtime", "1")
...
#        for col in cols:
        ...
        #ession_value = query_results.fetchall()
        #print(session_value_list[0])
        ...
        print(type(totals_query_results_row))
            #session_value_tuple = session_value_list[0]
            #session_value = session_value_tuple[0]

            #print(type(session_value))
            #if type(session_value) is int or type(session_value) is float: 

            #totals_value_list = query_results.fetchall()

            #for n in range(0, len(session_value_list)):
            #    if type(session_value_list[n]) is int or type(session_value_list[n]) is float:
